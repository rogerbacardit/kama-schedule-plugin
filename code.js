...
figma.ui.onmessage = async (msg) => {
    try {
        if (msg.type === 'get-credentials') {
            ...
            figma.ui.postMessage({ type: 'credentials', username: username || '', password: password || '' });
        }
            ...
            const { match, turnName, updateTime } = msg;
            // 1. Create main container frame
            const cardFrame = figma.createFrame();
            cardFrame.name = `MatchCard - ${match.homeTeam.shortName} vs ${match.awayTeam.shortName}`;
            ...
            badgeText.characters = "KINGS LEAGUE";
            ...
            const timeText = await createTextNode(headerFrame, `Actualizado: ${updateTime}`, 9, false, '#8E8E9C', 'RIGHT');
            ...
            if (hasScore) {
                ...
                if (match.scores.homeScoreP !== null || match.scores.awayScoreP !== null) {
                    const pensText = `PEN: (${match.scores.homeScoreP || 0} - ${match.scores.awayScoreP || 0})`;
                    await createTextNode(vsBlock, pensText, 9, true, '#FBB800', 'CENTER');
                }
            }
            else {
                // Render simple VS
            ...
            // Add time indicator
            const weekdayStr = msg.weekday || "TBD";
            const timeStr = msg.time || "TBD";
            ...
            // 5. Position and select on current page
            ...
            figma.notify(`¡Partido ${match.homeTeam.shortName} vs ${match.awayTeam.shortName} importado al lienzo!`);
        }
    }
    catch (err) {
        console.error("Figma Sandbox import error:", err);
        figma.notify(`Error al importar partido: ${err.message || err}`);
    }
};
